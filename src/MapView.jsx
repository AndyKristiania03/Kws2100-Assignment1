  ...
  useEffect(() => {
    ...
    const map = new Map({
      ...
      view: new View({
        ...
        center: transform([8.4689, 60.472], 'EPSG:4326', 'EPSG:3857'), // Center on Europe, converted to EPSG:3857
        zoom: 4,
      }),
    })
    ...
    >
      ...
      {selectedFeature && (
        ...
        >
          <h3>Information</h3>
          {/* display name & url */}
          {!!selectedFeature.get('navn') && (
            <p>
              <strong>Name:</strong> {selectedFeature.get('navn')}
            </p>
          )}
          {!!selectedFeature.get('url') && (
            <p>
              <strong>URL:</strong> {selectedFeature.get('url')}
            </p>
          )}
          {!!selectedFeature.get('category') && (
            <p>
              <strong>Category:</strong> {selectedFeature.get('category')}
            </p>
          )}

          {/* display address details */}
          {!!selectedFeature.get('romnr') && (
            <p>
              <strong>Room No:</strong> {selectedFeature.get('romnr')}
            </p>
          )}
          {!!selectedFeature.get('plasser') && (
            <p>
              <strong>Place:</strong> {selectedFeature.get('plasser')}
            </p>
          )}
          {!!selectedFeature.get('adresse') && (
            <p>
              <strong>Address:</strong> {selectedFeature.get('adresse')}
            </p>
          )}
          ...
          {selectedFeature.getGeometry()?.getType() === 'Point' && (
            <p>
              ...
              {JSON.stringify(selectedFeature.getGeometry().getCoordinates())}
            </p>
          )}
        </div>
      )}
